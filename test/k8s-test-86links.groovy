  ...
  node("k8s-master"){
        ...
        stage('版本处理') {
          ...
          stage('build') {
              ...
              dir("/home/jenkins/workspace/docker-test/$service_name"){ 
                sh(script:
                '''if [ -n "$service_name" ];then
                        rm -rf $service_name
                        cp /home/jenkins/workspace/k8s-test-86links/$service_name/build/libs/*.jar ./
                 else
                        cp /home/jenkins/workspace/k8s-test-86links/$service_name/build/libs/*.jar ./
                  fi    
                '''
            ...
            stage('docker编译') {
                sh "pwd"
                sh(script: """
                docker build -f Dockerfile -t 192.168.0.189/test/$service_name:$full_tag .""")
                sh "docker login -u admin -p Harbor12345 192.168.0.189"
                sh (script: "docker push 192.168.0.189/test/$service_name:$full_tag")
            }
          }
      }
        stage('k8s部署'){
                dir("/home/jenkins/workspace/helm-test"){
               ...
               sh(script: 
                ...
                """
                if [  -n "\$(helm list -n test | grep ${service_name})" ]; then
                    helm upgrade  --set image.tag=${full_tag}  ${service_name}  ${service_name}/ -n test
    
                else
                    helm install  --set image.tag=${full_tag}  ${service_name}  ${service_name}/ -n test
                fi
                """
                )
